...
export const getContent = cache(async (): Promise<ContentRecord> => {
  ...
  if (!snapshot.exists) return { content: defaultContent, version: 0, updatedAt: null, updatedBy: null };
  const data = snapshot.data()!;
  return { content: contentSchema.parse(data.content), version: data.version, updatedAt: data.updatedAt, updatedBy: data.updatedBy };
});
